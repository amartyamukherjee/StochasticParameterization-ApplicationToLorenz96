...
def huber_weight(residual, c):
  if np.abs(residual) > c:
    return c/np.abs(residual)
  else:
    return 1

for i in range(10):
  new_model = np.polyfit(x_1D_vals, u_1D_vals, 4, w=np.vectorize(huber_weight)(u_1D_vals-polynomial_regression(x_1D_vals), c))
