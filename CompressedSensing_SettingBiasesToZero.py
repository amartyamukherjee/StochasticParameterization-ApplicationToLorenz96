...
new_output = run_lorenz96_matrix_truth(x_shrunken[prediction_start_interval, :], xi, Dt, x_shrunken.shape[0]-prediction_start_interval, burn_in, skip)
new_x = new_output[0]

############## Predict the trajectory for 3 * error_doubling_time ##############
prediction_time = 0.9

new_output_pred = run_lorenz96_matrix_truth(x_shrunken[end_interval, :], xi, Dt, int(prediction_time/Dt), burn_in, skip)
