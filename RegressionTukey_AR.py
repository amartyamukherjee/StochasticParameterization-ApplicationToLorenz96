...
def tukey_biweight(residual, c):
  if np.abs(residual) <= c:
    return (1-(residual/c)**2)**2
  return 0

for i in range(10):
  new_model = np.polyfit(x_1D_vals, u_1D_vals, 4, w=np.vectorize(tukey_biweight)(u_1D_vals-polynomial_regression(x_1D_vals), c))
...
errors = u_shrunken - polynomial_regression(x_shrunken)
error0 = errors[:-1,:]
error1 = errors[1:,:]
